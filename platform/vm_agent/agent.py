...
def _build_interact_system_prompt() -> str:
    return (
        ...
        "All tools below are pre-approved and auto-approved. Use them directly.\n"
        "DO NOT ask the user to approve, whitelist, or grant permission for anything.\n"
        "DO NOT mention settings.json, permissions, or approval in your response.\n"
        "If a tool call is denied, retry it — do not ask the user to fix permissions.\n\n"
        f"Wolfram:  {WOLFRAM_PATH} -script <file.wls>\n"
...
def interact_query(body: dict, _=Depends(_auth)):
    ...
    if stored_hash != prompt_hash:
        ...
        try:
            ...
            pass  # hash file write failed — session still cleared, will retry next query
